...
(function () {
	...
	$(function(){
		...
		document.onkeydown = function(event) {
			...
			// Verifica si el foco está en un campo de texto
			var isInputFocused = event.target.tagName === "INPUT" || event.target.tagName === "TEXTAREA";
		
			// Bloquea F12 (herramientas de desarrollo), Ctrl+Shift+I (inspeccionar), y Ctrl+U (ver código fuente)
			if (
				key === "F12" || // Bloquea F12
				(event.ctrlKey && event.shiftKey && key === "I") || // Bloquea Ctrl+Shift+I
				(event.ctrlKey && key === "U") // Bloquea Ctrl+U
			) {
				event.preventDefault();
				return false;
			}
		
			// Si no está en un campo de texto, bloquear teclas normales
			if (!isInputFocused) {
				event.preventDefault();
				return false;
			}
		
			// Permitir las teclas normales en los campos de texto
			return true;
		};
	});
}());